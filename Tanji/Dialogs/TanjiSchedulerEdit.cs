...
namespace Tanji.Dialogs
{
    public partial class TanjiSchedulerEdit : Form
    {
        ...
        private void PacketTxt_TextChanged(object sender, EventArgs e)
        {
            ...
            bool isCorrupted = (packet.IsCorrupted || packet.Protocol != Main.Game.Protocol);
            const string PacketInfoFormat = "Header: {0} | Length: {1} | Corrupted:";

            IsCorruptedLbl.Text = isCorrupted.ToString();
            PacketInfoLbl.Text = string.Format(PacketInfoFormat, isCorrupted ? 0 : packet.Header, packet.Length);

            IsCorruptedLbl.Location = new Point((PacketInfoLbl.Location.X + PacketInfoLbl.Width) - 5, IsCorruptedLbl.Location.Y);
            IsCorruptedLbl.ForeColor = isCorrupted ? Color.Firebrick : SystemColors.HotTrack;

            UpdateBtn.Enabled = !isCorrupted;
        }
